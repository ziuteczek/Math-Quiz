...
const generateQuizNumbers = function(quizLength,minNumber,maxNumber,canBeNegative)
{
  const quizNumbers = [];
...
export function quizQuestionGenerator(operation,difficulty,length,canBeNegative){
  const quizNumbers = quizDiffHandler(difficulty,length,canBeNegative=true);
  const quizData = 
  {
    numbers: quizNumbers,
    operation: operation,
